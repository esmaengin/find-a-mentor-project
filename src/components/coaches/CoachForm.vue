<template>
    <form @submit.prevent="submitForm">
        <div class= "form-control" :class="{invalid: !firstName.isValid}">
            ...
            <p v-if="!firstName.isValid">Ad bilgisi boş geçilemez!</p>
        </div>
        <div class= "form-control" :class ="{invalid: !lastName.isValid}">
            <label for="lastname">Soyad</label>
            <input type="text" id="lastname" v-model.trim="lastName.val" @blur="clearValidity('lastName')"/>
            <p v-if="!lastName.isValid">Soyad bilgisi boş geçilemez!</p>
        </div>
        <div class= "form-control" :class ="{invalid: !description.isValid}">
            <label for="description">Açıklama</label>
            <textarea id="description" rows="5" v-model.trim="description.val" @blur="clearValidity('description')"></textarea>
            <p v-if="!description.isValid">Açıklama bilgisi boş geçilemez!</p>
        </div>
        <div class= "form-control" :class ="{invalid: !rate.isValid}">
            <label for="rate">Saatlik Ücret</label>
            <input type="number" id="rate" v-model.number="rate.val" @blur="clearValidity('rate')"/>
            <p v-if="!rate.isValid">Ücret bilgisi 0'dan büyük olmalıdır!</p>
        </div>
        <div class="form-control" :class="{invalid: !areas.isValid}">
            ...
            <div>
                <input type="checkbox" id="engineering" value="engineering" v-model="areas.val" @blur="clearValidity('areas')">
            ...
            <div>
             ...
             <p v-if="!areas.isValid">En az bir tane uzmanlık alanı seçmelisiniz!</p>
        </div>
        <p v-if="!formIsValid">Lütfen istenen bütün bilgileri doldurun</p>
        <base-button>Başvur</base-button>
    </form>
</template>
